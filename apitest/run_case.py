...
from apitest.A_WQRFhtmlRunner import HTMLTestRunner
from apitest.common.handles import *
    ...
    def demo(self, step):
        ...
        if mock_res not in ['', None, 'None']:
            ...
            if api_body_method == 'none' or api_body_method == 'null':
                response = request_handle(api_method, url, data={}, header=header, files='')
                ...
                for i in eval(api_body):
                    payload[i[0]] = i[1]
                response = request_handle(api_method, url, payload, header, files='')
        ...
        if assert_qz != '':
            for i in assert_qz.split('\n'):
                if i not in res:
                    raise AssertionError(f'字符串不存在：{i}')
...
def run(Case_id, Case_name, steps):
    ...
    filename = f'apitest/templates/Reports/{Case_id}.html'
    with open(filename, 'wb') as fp:
        runner = HTMLTestRunner(fp, title=f'接口测试平台测试报告：{Case_name}', description='用例描述')
        runner.run(suit)
